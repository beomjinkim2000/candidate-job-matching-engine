...
class TestLoadGuardrails:
    ...
    # --- docs/.guardrails 선별 ---
    # 없으면 전부(위 테스트들이 그 동작을 덮는다), 있으면 적힌 것만.
    # 이 프로젝트에서 전부는 107,806자(~72k 토큰)였고 step 14개면 100만 토큰이다.

    def test_manifest_selects_only_listed(self, executor, tmp_project):
        (tmp_project / "docs" / ".guardrails").write_text("arch.md\n")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "# Architecture" in result
        assert "# Guide" not in result       # 목록에 없으면 안 들어간다
        assert "# Rules" in result           # CLAUDE.md는 항상 들어간다

    def test_manifest_ignores_comments_and_blanks(self, executor, tmp_project):
        (tmp_project / "docs" / ".guardrails").write_text(
            "# 주석만 있는 줄\n\narch.md   # 뒤 주석\n\n"
        )
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
    ...
    def test_manifest_missing_file_warns_and_skips(self, executor, tmp_project, capsys):
        (tmp_project / "docs" / ".guardrails").write_text("NOPE.md\narch.md\n")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "# Architecture" in result
        assert "NOPE" in capsys.readouterr().out   # 조용히 넘어가지 않는다

    def test_empty_manifest_yields_only_claude_md(self, executor, tmp_project):
        (tmp_project / "docs" / ".guardrails").write_text("# 전부 뺀다\n")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "# Rules" in result
        assert "# Architecture" not in result
