...
판단 채점 (judge · 심사위원 2명, 이견 시 3번째)
   ↓
검산 (model)        ← 하나라도 걸리면 결과가 안 나간다. G1~G5 + G7. G6은 미구현
   ↓
합산 · 랭킹 (pipeline)
```
...
`model`이 결과를 내보내기 전에 검산을 돌린다. 개발 중에만 도는 것이 아니라
**매 실행마다** 돈다. **G1~G5는 `governance.py`, G7은 `source/registry.py`가 본다.**
**G6(판단유탈 대장)은 미구현이다** — 의존하는 `pipeline/ledger.py`를 못 만들었다. 특히 **G4(모든 조건에 이미지 좌표가 있는가)가
