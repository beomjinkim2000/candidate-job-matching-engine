...
실패하는 것을 실물로 보여주는 공고**이고, 그게 `step3.md` 3-C가 사전 대신 LLM에 문자열만
보내기로 한 결정의 **유일한 실증**이다. 표준 헤더만 쓰는 공고 두 개를 골랐다면
그 결정은 끝까지 가정으로 남는다.
...
| 2.7 | GitHub public 레포 생성·push | cc:TODO | 키·`.claude` 미포함 검사 + **수록 범위 확인**. 무엇을 싣고 무엇을 빼는지는 `step0.md`「공개 레포에 무엇을 싣는가」가 정본. 요약: 코드·테스트·목업 데이터·`docs/` 설계 근거는 **공개**, `phases/`·`Plans.md`·`docs/HARNESS_*.md`·`scripts/`는 **제작 과정 메타데이터라 제외** |
