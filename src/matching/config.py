...
class Settings:
    ...
    def __repr__(self) -> str:
        ...
        마스킹 문자열에 **실제 키의 접두사를 흉내 내지 않는다** — step 0 AC가
        `repr(s)`에 그 접두사가 없는 것으로 누출을 검사하므로, 가짜 접두사를 쓰면
        **검사 자체가 무력해진다.** 같은 이유로 이 파일에도 접두사를 적지 않는다:
        step 8 검증 절차가 `grep "<접두사>" src/`로 키가 코드에 박혔는지 훑는데,
        설명에 적어 두면 그 검사가 주석에 걸려 아무 말도 못 하게 된다.
        """
        shown = {
