...
**이 규칙은 「여기 온 조건은 이름으로 찾을 수 있다」를 전제한다.** 전제가 깨지면 산문
조건에서 연도·한글 덩어리를 뽑아 대조하게 되고, 그건 **조건의 뜻을 재는 것이 아니다.**

전제를 세우는 것은 이 파일이 아니라 `rubric/branch.py`다 — 조건이 `term` 갈래일 때만
여기로 온다. 옛 규칙(`is_countable()`)은 숫자가 하나 끼면 산문 조건도 여기로 보냈고,
실측 공고에서 그렇게 온 조건이 표현 9개 중 3개만 맞아 0.33점을 받았다.
**그 오분류를 고친 자리가 갈래 분류이고, 여기는 그대로 두었다** — 이 규칙 자체는
「이름을 찾는다」는 제 일을 하고 있었다.

**LLM을 못 부르면 전제가 다시 헐거워진다** (폴백이 옛 규칙이다). 그때 무엇이 어떻게
정해졌는지는 `BranchResult.fell_back`과 결과의 `layer`·`branch`에 남는다.
"""
